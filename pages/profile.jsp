...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" integrity="sha512-MV7K8+y+gLIBoVD59lQIYicR65iaqukzvf/nwasF0nqhPay5w/9lJmVM2hMDcnK1OnMGCdVK+iQrJ7lzPJQd1w==" crossorigin="anonymous" referrerpolicy="no-referrer"/>
    <link rel="stylesheet" href="<%=request.getContextPath()%>/asset/Style/profile.css"/>
</head>
<body>
...
<%
        ...
        %>
	<form action="UpdateProfileServlet?emailid=<%=userDetails.getEmail() %>" id="profilepage" method="post">
		<div class="profile">
			...
			<div class="names">
				...
				<br /> <input type="tel" id="phone_number" name="phone"
					placeholder="phone number" pattern="^[6-9][0-9]{9}$" value="<%= userDetails.getPhonenumber() %>" required><br>
				<br> <input type="text" name="address" placeholder="address"
					id="address" class="text" pattern="[a-zA-Z0-9\s\-\,\#]+" value="<%= userDetails.getAddress() %>" required /><br>
				<br> <input type="text" name="city" placeholder="city"
					id="city" class="text" pattern="[A-Za-z]{1,24}" value="<%= userDetails.getCity() %>" required /><br>
				<br> <input type="text" name="state" placeholder="state"
					id="state" class="text" pattern="[A-Za-z]{1,30}" value="<%= userDetails.getState() %>" required /><br>
				<br> <input type="text" name="pincode" placeholder="pincode"
					id="pincode" class="text" pattern="[0-9]{6}" maxlength="6"  value="<%= userDetails.getPincode() %>" required /><br>
				<br>
			</div>
			...
			<img src="<%=request.getContextPath()%>/asset/IMG/home page/profile2.jpg" alt="empty profile" />
		</div>
	</form>
			<%
			}
